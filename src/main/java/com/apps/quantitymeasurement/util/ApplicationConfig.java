...
/**
 * DatabaseConfig is a singleton class responsible for loading and managing
 * database configuration properties. It supports loading configurations from a
 * properties file, system properties, or environment variables. The class
 * provides methods to retrieve configuration values and manage different
 * environments (development, testing, production).
 *
 * The configuration properties include database connection details such as
 * driver class, URL, username, password, and connection pool settings. The
 * class also defines enums for configuration keys and environments to ensure
 * type safety and avoid hardcoding strings.
 *
 * @author Developer
 * @version 16.0
 * @since 16.0
...
public class ApplicationConfig {
	...
	private void loadDefaults() {
		...
		properties.setProperty("db.url", "jdbc:mysql://127.0.0.1:3306/quantity_measurement");
		properties.setProperty("db.username", "root");
		properties.setProperty("db.password", "Arif@123");
